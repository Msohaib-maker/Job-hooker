...
import {
    User, Mail, Globe, Link2, FileText,
    GraduationCap, Briefcase, ChevronDown,
    X, Plus, Trash2, Loader2,
    Sparkles, ArrowRight, ArrowLeft, Award, Heart, Languages
} from "lucide-react";
...
export const SKILL_OPTIONS = [
    ...
    "Kubernetes", "AWS", "GCP", "Azure", "GraphQL", "REST APIs",
    "Git", "CI/CD", "Figma", "UI/UX Design", "Product Management",
    "Data Analysis", "Machine Learning", "SQL", "TailwindCSS", "Vue.js",
];
...
const labelClass = "block text-xs font-medium text-[#737373] mb-1.5 uppercase tracking-wide";
...
const ProfileDialog = ({  open, onClose, onSubmit }: ProfileDialogProps) => {
    ...
    const [form, setForm] = useState<ProfileForm>({
        name: "", email: "", website: "", otherLink: "",
        description: "", skills: [], education: [], interests: [], certificates: [], languages: [], experience: []
    });
    ...
    const filteredSkills = SKILL_OPTIONS.filter(s =>
        s.toLowerCase().includes(skillSearch.toLowerCase()) && !form.skills.some(sk => sk.name === s)
    );
    ...
    const set = (field: keyof ProfileForm, value: string | string[]|Experience[]|Education[]|SkillItem[]) =>
    ...
    const removeSkill = (skillName: string) =>
        set("skills", form.skills.filter(s => s.name !== skillName));
    ...
    /* EDUCATION CONTROLS */
    const addEducation = () =>
        set("education", [...form.education, { institution: "", degree: "", grade: "" }]);

    const updateEducation = (i: number, field: keyof Education, value: string) => {
        const updated = [...form.education];
        if (field === "grade") {
            // Guardrail to keep grade clean within strings representing 0-100
...
const updateExperience = (i: number, field: keyof Experience, value: string) =>
  setForm(f => ({
    ...f,
    experience: f.experience.map((exp, idx) => idx === i ? { ...exp, [field]: value } : exp)
  }));

    /* ARRAY TAG CONSTRUCTORS (Interests, Certificates, Languages) */
    const addTagItem = (field: "interests" | "certificates" | "languages", value: string, clearInput: () => void) => {
        if (!value.trim()) return;
        set(field, [...form[field], value.trim()]);
        clearInput();
    };

    const removeTagItem = (field: "interests" | "certificates" | "languages", index: number) => {
        set(field, form[field].filter((_, idx) => idx !== index));
    };
    ...
    const validateStep2 = () => {
        const e: Record<string, string> = {};
        if (form.skills.length === 0) e.skills = "Select at least one skill & level";
            ...
            <div className="relative w-full max-w-2xl bg-[#111111] border border-[#262626] rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div className="flex items-center justify-between px-8 py-6 border-b border-[#262626]">
                    <div>
                        ...
                        <p className="text-xs text-[#525252] mt-0.5">Used by AI to generate customized artifacts</p>
                    </div>
                    <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg border border-[#262626] text-[#737373] hover:text-[#EDEDED] hover:border-[#525252] transition">
                        <X className="w-4 h-4" />
                    </button>
                </div>
                        ...
                        <>
                            ...
                            <section>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label className={labelClass}>Name <span className="text-[#C4F029]">*</span></label>
                            ...
                            <section>
                                ...
                                <label className={labelClass}>About you <span className="text-[#C4F029]">*</span></label>
                                <textarea
                                    rows={5}
...
<section>
  ...
  <div className="flex flex-col gap-3">
    {form.experience.map((exp, i) => (
      <div key={i} className="bg-[#1A1A1A] border border-[#262626] rounded-xl p-4 flex flex-col gap-3">
    ...
    >
      <Plus className="w-4 h-4" /> Add Experience
                                    ...
                                    {form.education.map((edu, i) => (
                                        <div key={i} className="bg-[#1A1A1A] border border-[#262626] rounded-xl p-4 flex flex-col gap-3">
                                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                                <div>
                                                    <input className={inputClass} placeholder="Institution" value={edu.institution}
                                                ...
                                                <div>
                                                    <input className={inputClass} placeholder="Grade (0-100)" value={edu.grade} type="text" maxLength={3}
                                                        onChange={e => updateEducation(i, "grade", e.target.value)} />
                                                    {errors[`edu_${i}`] && <p className="text-red-400 text-[10px] mt-1">{errors[`edu_${i}`]}</p>}
                                                </div>
                                            </div>
                            ...
                            <section>
                                ...
                                <label className={labelClass}>Add Professional Skills <span className="text-[#C4F029]">*</span></label>
                                <div ref={skillRef} className="relative mb-4">
                                    ...
                                    >
                                        ...
                                        />
                                        <ChevronDown className={`w-4 h-4 text-[#525252] transition-transform ${skillOpen ? "rotate-180" : ""}`} />
                                    </div>
                                ...
                                {errors.skills && <p className="text-red-400 text-xs mb-3">{errors.skills}</p>}
                                ...
                                {form.skills.length > 0 && (
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-[#161616] p-4 rounded-xl border border-[#262626]">
                                        {form.skills.map((item, idx) => (
                                            <div key={item.name} className="flex flex-col gap-1.5 p-2 bg-[#1A1A1A] rounded-lg border border-[#262626]">
                                                <div className="flex justify-between items-center">
                                                    <span className="text-xs font-semibold text-[#EDEDED]">{item.name}</span>
                                                    <button onClick={() => removeSkill(item.name)} className="text-[#525252] hover:text-red-400 transition">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <input 
                                                        ...
                                                        onChange={(e) => updateSkillExpertise(idx, parseInt(e.target.value))}
                            ...
                            <section>
                                ...
                                <div className="flex gap-2 mb-3">
                                    <input 
                                        className={inputClass} placeholder="e.g. Open Source, Cloud Architecture, Quantum Computing" 
                                        value={interestInput} onChange={e => setInterestInput(e.target.value)}
                                        onKeyDown={e => e.key === 'Enter' && addTagItem("interests", interestInput, () => setInterestInput(""))}
                                    />
                                    <button onClick={() => addTagItem("interests", interestInput, () => setInterestInput(""))} className="px-4 rounded-xl bg-[#262626] hover:bg-[#323232] text-sm text-[#EDEDED] border border-[#383838]">Add</button>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                    {form.interests.map((interest, i) => (
                                        <span key={i} className="flex items-center gap-1.5 bg-[#262626] text-xs text-[#EDEDED] px-3 py-1 rounded-full border border-[#383838]">
                                            {interest}
                                            <button onClick={() => removeTagItem("interests", i)} className="text-[#737373] hover:text-red-400"><X className="w-3 h-3" /></button>
                                        </span>
                                    ))}
                                </div>
                            </section>
                            ...
                            <section>
                                ...
                                <div className="flex gap-2 mb-3">
                                    <input 
                                        className={inputClass} placeholder="e.g. AWS Certified Solutions Architect, PMP" 
                                        value={certInput} onChange={e => setCertInput(e.target.value)}
                                        onKeyDown={e => e.key === 'Enter' && addTagItem("certificates", certInput, () => setCertInput(""))}
                                    />
                                    <button onClick={() => addTagItem("certificates", certInput, () => setCertInput(""))} className="px-4 rounded-xl bg-[#262626] hover:bg-[#323232] text-sm text-[#EDEDED] border border-[#383838]">Add</button>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                    {form.certificates.map((cert, i) => (
                                        <span key={i} className="flex items-center gap-1.5 bg-[#262626] text-xs text-[#EDEDED] px-3 py-1 rounded-full border border-[#383838]">
                                            {cert}
                                            <button onClick={() => removeTagItem("certificates", i)} className="text-[#737373] hover:text-red-400"><X className="w-3 h-3" /></button>
                                        </span>
                                    ))}
                                </div>
                            </section>
                            ...
                            <section>
                                ...
                                <div className="flex gap-2 mb-3">
                                    <input 
                                        className={inputClass} placeholder="e.g. English (Fluent), Spanish (Native)" 
                                        value={langInput} onChange={e => setLangInput(e.target.value)}
                                        onKeyDown={e => e.key === 'Enter' && addTagItem("languages", langInput, () => setLangInput(""))}
                                    />
                                    <button onClick={() => addTagItem("languages", langInput, () => setLangInput(""))} className="px-4 rounded-xl bg-[#262626] hover:bg-[#323232] text-sm text-[#EDEDED] border border-[#383838]">Add</button>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                    {form.languages.map((lang, i) => (
                                        <span key={i} className="flex items-center gap-1.5 bg-[#262626] text-xs text-[#EDEDED] px-3 py-1 rounded-full border border-[#383838]">
                                            {lang}
                                            <button onClick={() => removeTagItem("languages", i)} className="text-[#737373] hover:text-red-400"><X className="w-3 h-3" /></button>
                                        </span>
                                    ))}
                                </div>
                            </section>
